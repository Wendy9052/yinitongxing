...
<script>
  ...
  export default {
    ...
    async created() {
      ...
      this.papid = papid
      this.is_medical_examined = pap.phase.is_medical_examined
      if (this.status) {
        Dialog.alert({
      ...
      if (this.patientOne) {
        if (this.patientOne.signature_image && this.status != '未上传') this.signature_image = this.patientOne.signature_image
