          ...
          <van-cell-group :title="text" :border="false">
            <van-cell v-for="(citem, cindex) in item.data" :key="cindex" is-link @click="goUpload(citem, item.name)">
              <template #title>
                ...
                <p style="color: red;">{{citem.words}}</p>
              </template>
              <div>
                <span
...
<script>
  ...
  export default {
    ...
    async mounted() {
      ...
      if (this.$route.query.category && this.$route.query.category == '后续复核') {
    ...
    methods: {
      ...
      async getData() {
        ...
        this.checkItems = await GetCheckItem({
          papid: this.papid,
      ...
      async goNext() {
        ...
        for (let i in item) {
          ...
          } else if (ele.status == "审核不通过" && ele.is_required == "1") {
            Toast('您有驳回的材料，请重新上传')
            allup = false
            return
          }
