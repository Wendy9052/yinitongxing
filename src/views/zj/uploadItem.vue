...
<script>
  import Vue from 'vue';
  import { FollowUpload, UploadRecipel } from '@/api/api'
  import {
    zjUploadCheckItem,
    zjdeleteCheckItem,
    zjGetCheckItem,
  ...
  export default {
    ...
    methods: {
      async getData() {
        ...
        this.checkItem = await zjGetCheckItem(params)
        this.checkItem.name = this.$route.query.name
        Toast.clear()
      },
      submitUpload() {
        ...
        if(this.checkItem.status == '审核不通过' && ci.length == 0) {
          Toast("您有驳回的材料，请重新上传");
          return;
        }
        Toast.loading({
        ...
        // } else {
          ...
          zjUploadCheckItem(params).then((res) => {
      ...
      back() {
        ...
        if (this.category == '后续复核' || this.category == '入组医学审核') {
